...
class UNetDecoder(nn.Module):
    def __init__(self, in_channels):
        ...
        self.up1 = self.up_block(in_channels, 512)
        self.up2 = self.up_block(512, 256)
        self.up3 = self.up_block(256, 128)
        self.up4 = self.up_block(128, 64)
        self.final_conv = nn.Conv2d(64, 1, kernel_size=1)  # 1通道输出，适用于二分类分割任务

    def up_block(self, in_channels, out_channels):
        return nn.Sequential(
            nn.ConvTranspose2d(in_channels, out_channels, kernel_size=2, stride=2),  # 替换 Upsample
    ...
    def forward(self, x):
        ...
        x = self.up4(x)
        x = self.final_conv(x)
        x = torch.sigmoid(x)  # 归一化到 [0, 1]，适用于 mask 预测
        return x
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False):
        ...
        self.unet_decoder = UNetDecoder(512 * block.expansion)
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        x = self.layer4(x)  # 提取特征

        feature_map = x  # Grad-CAM 使用的特征图
    
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        classification_output = self.fc(x)  # 分类任务

        # 通过 UNet 生成伪造区域分割图
        segmentation_output = None
        if self.unet_decoder is not None:
            segmentation_output = self.unet_decoder(feature_map)

        return classification_output, feature_map, segmentation_output  # 返回分类、Grad-CAM特征图、UNet 预测分割图
